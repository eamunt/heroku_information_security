...
def bin2dec(binary):
    ...
    decimal, i, n = 0, 0, 0
    while(binary != 0):
        dec = binary % 10 # Lấy bit cuối
        decimal = decimal + dec * pow(2, i)
        binary = binary//10 # Bỏ bit cuối
        i += 1
    return decimal
 
# Đổi Thập phân sang Nhị phân 
def dec2bin(num):
    res = bin(num).replace("0b", "") # Bỏ đi 0b
    # Chiều dài chuỗi không chia hết cho 4
    if(len(res)%4 != 0):
        div = len(res) / 4
        div = int(div)
        counter =(4 * (div + 1)) - len(res) # Xác định số lượng bit 0 thêm vào đầu
        for i in range(0, counter):
            res = '0' + res # Thêm bit 0 vào đầu chuỗi
    return res
...
def shift_left(k, nth_shifts):
    ...
    for i in range(nth_shifts):
        for j in range(1,len(k)): # Bit thứ 2 đến cuối
            s = s + k[j]
        s = s + k[0] # Bit đầu tiên dịch về vị trí cuối
        k = s
        s = ""
    return k   
...
def encryptDES(pt, key):
    ...
    rkb = [] # Bảng khóa con dạng nhị phân
    # Sinh khóa con cho 16 vòng
    for i in range(0, 16):
    ...
    pt = hex2bin(pt) # Chuyển plain text thành nhị phân
    ...
    for i in range(0, 16):
        ...
        # 8 S-boxes
        for j in range(0, 8):
            # Xét từng chuỗi 6 bits
            row = bin2dec(int(xor_x[j * 6] + xor_x[j * 6 + 5])) # Ghép bit đầu và cuối
            col = bin2dec(int(xor_x[j * 6 + 1] + xor_x[j * 6 + 2] +
xor_x[j * 6 + 3] + xor_x[j * 6 + 4])) # 4 bits giữa
            val = sbox[j][row][col] # Hàng row cột col
            sbox_str = sbox_str + dec2bin(val) # Đầu ra là 4 bits
...
def decryptDES(ct, key):
    ...
    ct = hex2bin(ct) # Chuyển plain text thành nhị phân
...
print("Encryption")
cipher_text = encrypt(pt, k)
print("Cipher Text : ",cipher_text)
 
print("Decryption")
text = decrypt(cipher_text, k)
